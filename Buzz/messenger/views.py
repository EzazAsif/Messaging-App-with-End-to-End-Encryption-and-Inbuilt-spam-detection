...
from .spamdetector import makepreds
from .encryptor import encrypt_message,decrypt_message
from .models import Messages
from .msgfuncs import chat_user,get_chat1
...
def chat(request, id=None):
    ...
    if id:
        ...
        chatuser = user[0]
        id = user[0].id    
    messages=get_chat1(request,chatuser.id)
    if request.method == "POST":
    ...
    return render(request, "chat.html", context={'users': user, 'chatuser': chatuser, 'chatusers': chatusers,'messages':messages})
